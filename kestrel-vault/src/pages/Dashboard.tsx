...
function SecurityScoreGauge({ score }: { score: number }) {
  ...
  return (
    <svg width="140" height="140" viewBox="0 0 140 140">
      ...
      />
      <text x="70" y="65" textAnchor="middle" fill="#0F172A" fontSize="28" fontWeight="600">{score}</text>
      <text x="70" y="82" textAnchor="middle" fill="#64748B" fontSize="11">/100</text>
    </svg>
  )
}
    ...
    <div className="animate-fade-in">
      ...
      <div className="p-8 space-y-6">
        <div className="grid grid-cols-4 gap-4">
          <div className="rounded-xl p-5 transition-shadow duration-150 hover:shadow-md"
            ...
            <div className="space-y-1">
              {entries.slice(0, 5).map((item) => (
                ...
                >
                  ...
                  <span className="text-sm flex-1 truncate" style={{ color: '#0F172A' }}>{item.title}</span>
                  <span className="text-xs flex-shrink-0" style={{ color: '#94A3B8' }}>{item.username}</span>
                </div>
              ))}
            </div>
          </div>
